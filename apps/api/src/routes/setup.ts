...
const CodexAuthResetResponseSchema = z
  .object({
    reset: z.literal(true),
  })
  .describe("Successful Codex auth reset result");
const CodexAuthLoginStatusSchema = z
  .object({
    state: z.string(),
    canImport: z.boolean(),
  ...
  .describe("Guided status for a managed Codex login session");
const CodexAuthAccountResponseSchema = z
  .object({
    account: z
      ...
      .nullable(),
    login: CodexAuthLoginStatusSchema,
  })
...
const CODEX_AUTH_IMPORT_RATE_LIMIT = {
  max: 60,
  timeWindow: "15 minutes",
};
...
export async function setupRoutes(rawApp: FastifyInstance) {
  ...
  const serializeCodexAccount = (
    account: Awaited<ReturnType<typeof getCodexAuthLoginStatus>>["account"],
  ) =>
    account
      ? {
          id: account.id,
  ...
  app.get(
    ...
    async (req, reply) => {
      const result = await getCodexAuthLoginStatus({
        workspaceId: req.user?.workspaceId ?? null,
        userId: req.user?.id,
      });
      reply.send({ account: serializeCodexAccount(result.account), login: result.login });
    },
  );
  ...
  app.post(
    ...
    {
      config: { rateLimit: CODEX_AUTH_IMPORT_RATE_LIMIT },
      preHandler: [requireAdminWhenAuthenticated],
      schema: {
  ...
  app.post(
    "/api/setup/codex-auth/reset",
    {
      config: { rateLimit: SETUP_POST_RATE_LIMIT },
      preHandler: [requireAdminWhenAuthenticated],
      schema: {
        operationId: "resetCodexAuthAccount",
        summary: "Reset managed Codex auth state",
        description:
          "Clear the stored shared Codex auth blob and reset the app-managed " +
          "Codex account so a fresh managed login can be started cleanly.",
        tags: ["Setup & Settings"],
        response: { 200: CodexAuthResetResponseSchema, 400: ErrorResponseSchema },
      },
    },
    async (req, reply) => {
      try {
        await resetCodexAuthAccount({
          workspaceId: req.user?.workspaceId ?? null,
        });
        reply.send({ reset: true as const });
      } catch (err) {
        reply.status(400).send({ error: sanitizeError(err) });
      }
    },
